					...
					for valType := chkItr.Next(); valType != chunkenc.ValNone; valType = chkItr.Next() {
						...
						case chunkenc.ValFloat:
							...
							if samples != nil {
								...
								require.Fail(t, "received float samples but none were expected")
							}
						case chunkenc.ValHistogram:
							...
							if histograms != nil {
								...
								if chkIdx > 0 {
									// This is set when the histogram is returned for any sample that isn't the first sample in the chunk.
									// https://github.com/prometheus/prometheus/blob/001ee2620e094970e5657ce39275b2fccdbd1359/tsdb/chunkenc/histogram_meta.go#L463-L466
									expected.CounterResetHint = histogram.NotCounterReset
								}
								require.Equal(t, expected, h)
							} else {
								require.Fail(t, "received histogram samples but none were expected")
							}
